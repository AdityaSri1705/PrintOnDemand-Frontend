...
export default function HabitsPage({updatePriceBox}) {
  ...
  useEffect(() => {
    ...
    axios.get(`${BACKEND_URL}/api/V1/addins/Habits`)
      .then(response => {
    ...
    if(sessionData!==null){
      ...
      const HebaitsList = sessionData.Habits;

      if(HebaitsList!==undefined){
        setTemplateSelected((HebaitsList)? HebaitsList:[]);
        //show images of select template
        const index1 = HebaitsList.findIndex((item) => item.id === HebaitsList[0].templateId);
  ...
  useEffect(() => { 
    ...
    if(NewSessionData!==null){
      NewSessionData['Habits']=templateSelected;
      ...
      <Box className="PageInnerBox">
        <Box className="LeftPanelBox">
          <Box className="LeftHeader">
            <Typography className='LeftTitle'>Habits</Typography>
          </Box>
