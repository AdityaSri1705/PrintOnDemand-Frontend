...
export default function FamilyPage({updatePriceBox}) {
  ...
  useEffect(() => {
    ...
    axios.get(`${BACKEND_URL}/api/V1/addins/Family`)
      .then(response => {
    ...
    if(sessionData!==null){
      ...
      const FamilyList = sessionData.Family;

      if(FamilyList!==undefined){
        setTemplateSelected((FamilyList)? FamilyList:[]);

        //show images of select template
        const index1 = FamilyList.findIndex((item) => item.id === FamilyList[0].templateId);
  ...
  useEffect(() => { 
    ...
    if(NewSessionData!==null){
      NewSessionData['Family']=templateSelected;
      ...
      <Box className="PageInnerBox">
        <Box className="LeftPanelBox">
          <Box className="LeftHeader">
            <Typography className='LeftTitle'>Family</Typography>
          </Box>
