...
export default function FitnessPage({updatePriceBox}) {
  ...
  useEffect(() => {
    ...
    axios.get(`${BACKEND_URL}/api/V1/addins/Fitness-and-Food`)
      .then(response => {
    ...
    if(sessionData!==null){
      ...
      const FitnessList = sessionData.FitnessFood;

      if(FitnessList!==undefined){
        setTemplateSelected((FitnessList)? FitnessList:[]);

        //show images of select template
        const index1 = FitnessList.findIndex((item) => item.id === FitnessList[0].templateId);
  ...
  useEffect(() => { 
    ...
    if(NewSessionData!==null){
      NewSessionData['FitnessFood']=templateSelected;
      ...
      <Box className="PageInnerBox">
        <Box className="LeftPanelBox">
          <Box className="LeftHeader">
            <Typography className='LeftTitle'>Fitness and Food</Typography>
          </Box>
