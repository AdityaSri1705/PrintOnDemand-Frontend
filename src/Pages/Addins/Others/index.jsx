...
export default function OthersPage({updatePriceBox}) {
  ...
  useEffect(() => {
    ...
    axios.get(`${BACKEND_URL}/api/V1/addins/Other`)
      .then(response => {
    ...
    if(sessionData!==null){
      ...
      const OthersList = sessionData.Others;

      if(OthersList!==undefined){
        setTemplateSelected((OthersList)? OthersList:[]);

        //show images of select template
        const index1 = OthersList.findIndex((item) => item.id === OthersList[0].templateId);
  ...
  useEffect(() => { 
    ...
    if(NewSessionData!==null){
      NewSessionData['Others']=templateSelected;
      ...
      <Box className="PageInnerBox">
        <Box className="LeftPanelBox">
          <Box className="LeftHeader">
            <Typography className='LeftTitle'>Others</Typography>
          </Box>
