...
export default function WorkPage({updatePriceBox}) {
  ...
  useEffect(() => {
    ...
    axios.get(`${BACKEND_URL}/api/V1/addins/Work`)
      .then(response => {
    ...
    if(sessionData!==null){
      ...
      const WorkList = sessionData.Work;

      if(WorkList!==undefined){
        setTemplateSelected((WorkList)? WorkList:[]);

        //show images of select template
        const index1 = WorkList.findIndex((item) => item.id === WorkList[0].templateId);
  ...
  useEffect(() => { 
    ...
    if(NewSessionData!==null){
      NewSessionData['Work']=templateSelected;
      ...
      <Box className="PageInnerBox">
        <Box className="LeftPanelBox">
          <Box className="LeftHeader">
            <Typography className='LeftTitle'>Work</Typography>
          </Box>
