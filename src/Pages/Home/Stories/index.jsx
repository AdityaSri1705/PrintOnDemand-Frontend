...
export const handleBoxScroll = () => {
  const videosBox = document.getElementById('stories');
  videosBox.scrollIntoView({ behavior: 'smooth' });
};


export default function Stories({storyList}) {
  ...
  return (
    <>
      <Box className="storiesContainer " id="stories">
        <Box className="storiesHeader">
          <Typography>Stories</Typography>
        </Box>

        <Box className="storiesBox" ref={scrl} onScroll={scrollCheck}>


         {storyList.map((story, idx) => (
          <Box className={`storiesCard${idx>0 ? ' padding2nd' : ''}`}>
              ...
              <Box className="userBox">
                <Box mb={2} className="userInfoBox">
                  <Box mr={1.5} className="userImage">
                    <img src={story.profile_img} alt='profile_img' />
                  </Box>
                  <Box className="userTitle">
                    <Typography className='userName'>{story.first_name+" "+story.last_name}</Typography>
                    <Typography className='userLink'>{story.first_name}</Typography>
                  </Box>
                </Box>
                <Box className="userTextBox">
                  <Typography>{story.comments}</Typography>
                </Box>
              </Box>
            </Box>
          </Box>
        ...
        <Box className="buttonBox">
          <Box className={`ScrollBtn${scrolLeftEnd ? ' arrowDisabled' : ''}`}
            ...
            <img style={{ width: "40px" }} src={leftArrow} alt='leftArrow' />
          </Box>
          ...
          <Box className={`ScrollBtn${scrolEnd ? ' arrowDisabled' : ''}`}
            ...
            onMouseEnter={(e) => anim(e)}
            onMouseLeave={(e) => anim2(e)}>
            <img style={{ width: "40px" }} src={rightArrow} alt='rightArrow' />
          </Box>
