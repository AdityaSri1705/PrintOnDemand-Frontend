...
export default function Cart() {
    ...
    if(CartSessionData!==undefined && CartSessionData!==null){
      ...
      axios.get(`${BACKEND_URL}/api/V1/getCart/${CartSessionData.CartID}`,{ headers: ApiHeaders})
        .then(response => {
          ...
          const filteredItems = CartItems.map(({ id, quantity }) => ({ id, quantity }));
  ...
  const handleCheckout = () =>{
    ...
    if(isChecked){
      ...
      axios.post(`${BACKEND_URL}/api/V1/updateCart/`,
      {
        CartID: cartID,
      ...
      .then(response => {
        if(response.data.status){
          ...
          toast("Sorry! unable to checkout.",
          {
            position: "top-right",
            autoClose: 5000,
            hideProgressBar: false,
  ...
  const handleDeleteItem = (item_id) => {
    axios.post(`${BACKEND_URL}/api/V1/deleteCartItem`,
    {
      CartID: cartID,
      ItemId: item_id
    },
        ...
        <Box className="cartBox">
          ...
          <Box className="cartItemBox">
            ...
            {cartItems.map((item, index)=>(
              <CartItem itemData={item} key={`${index}`}  setItemQty={handleItemQty} deleteItem={handleDeleteItem}/>
            ))}
          ...
          <input
              ...
              checked={isChecked}
              type="checkbox"  />
            <Typography ml={1}>I have reviewed all the items in my cart and understand that my custom items will be printed as shown in the preview.</Typography>
          </Box>
