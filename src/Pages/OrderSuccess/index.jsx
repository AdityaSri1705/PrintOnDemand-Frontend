...
export default function OrderSuccess() {
  ...
  useEffect(() => {
    ...
    if(CartSessionData!==undefined && CartSessionData!==null){
      ...
      axios.get(`${BACKEND_URL}/api/V1/sendtoprint/${CartSessionData.CartID}`, { headers: ApiHeaders})
        .then(response => {
    ...
    if(window.friendbuyAPI.merchantId){
      window.friendbuyAPI.push(["track","page",{ name:"Order-Success"}]);
      console.log("Call2=>",window.friendbuyAPI, window.friendbuyAPI.merchantId)
    }
  },[]);
    ...
    <>
      ...
      <Box className="cartContainer">
        ...
        <Box className="cartTopLogo">
          ...
          <Typography className='cartHeaderSubtotal'>Order Successfull</Typography>
            
          {!showProcessing && <Box className="preLoader" >
         
            <Box className="PaymentArea" > 
        
              <Typography className='PaymentText1'>Please Wait...</Typography>
              <Typography className='PaymentText1'>We prcessing your order..</Typography>
                
            </Box>
          </Box>}

          {showProcessing && <Box className="PaymentArea" > 
              <Typography className='PaymentText3'>Thank you</Typography>
              <Typography className='PaymentText1'>Your order has been placed successfully.</Typography>
              <Box className="PaymentTextTab">
                <Typography mr={1} className='PaymentText1'>Your order id :</Typography>
                <Typography onClick={() => navigate('/myaccount')} className='PaymentText2'>{cartData.code}</Typography>
              </Box>
          </Box>}
